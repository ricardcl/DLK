...
export class AnalyseLPLN {
  ...
  constructor(parseurLPLN: ParseurLPLN, dates:Dates,split:Split,frequences:Frequences) {
    console.log("Je rentre dans le constructor AnalyseLPLN ");  
    this.parseurLPLN = parseurLPLN; 
    this.split =split;
    this.dates = dates;
  ...
  public analyse(arcid: string, plnid: number): Vol {
    console.log("Classe AnalyseLPLN Fonction analyse");
    const fichierGbdi = p.resolve(Path.systemPath, "STPV_G2910_CA20180816_13082018__1156");
    const source = p.resolve(this.parseurLPLN.getUserPath(), "resultLPLN.htm"); //Fichier en entree a analyser 
  ...
  public isVolEquipeCpdlc(): boolean {
    console.log("Classe AnalyseLPLN Fonction isVolEquipeCpdlc");
